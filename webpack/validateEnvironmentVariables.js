...
const validateEnv = (envConfig) => {
    ...
    if (!environmentVariables) {
        logErrorAndExit(
            'Failed to validate environment variables: No environment variable configuration found.'
        );
    }
    ...
    if (!requiredEnvVars || requiredEnvVars.length === 0) {
        console.log(
            `No required environment variables defined for application. Skipping validation.`
        );
        return;
    }
    ...
    for (const varName of requiredEnvVars) {
        if (!(varName in envConfig)) {
            logErrorAndExit(
                `Missing required environment variable: "${varName}". Please define it in your .env file.\nYou can refer to the .env.template file for guidance.`
            );
        }
    }
