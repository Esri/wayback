...
class MetadataManager {
    ...
    async queryData(params:IParamsQueryMetadata):Promise<IWaybackMetadataQueryResult>{
        ...
        try {
            ...
            const feature:IFeature = queryResponse.features && queryResponse.features.length ? queryResponse.features[0] : null;
    ...
    private getQueryUrl(releaseNum:number, zoom:number){
        const metadataLayerUrl = this.waybackconfig[releaseNum].metadataLayerUrl;
